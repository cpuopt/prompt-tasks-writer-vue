<template>
    ...
    >
        <div class="tag-local" :style="{ backgroundColor: bgColor }">
            <el-text v-if="!edit_local_status" class="text-local disabled-select">{{ tag.local }}</el-text>
            <el-input v-else size="small" v-model="local_input" style="width: 100%" @keyup.enter.native="save_local" @click.stop>
                <template #suffix>
                    <el-icon class="el-input__icon" style="cursor: pointer; color: #606266" @click.stop="save_local"><save /></el-icon>
                </template>
            </el-input>
        </div>
        <div class="tag-en">
            <el-text v-if="!edit_raw_status" class="disabled-select">{{ tag.raw }}</el-text>
            <el-input v-else size="small" v-model="raw_input" style="width: 100%" @keyup.enter.native="save_raw" @click.stop>
                <template #suffix>
                    <el-icon class="el-input__icon" style="cursor: pointer; color: #606266" @click.stop="save_raw"><save /></el-icon>
                </template>
            </el-input>
        </div>
        <div class="contextmenu" v-show="contex_status" @click.stop>
            <el-button class="button" type="primary" :icon="EditPen" circle plain @click.stop="edit_tag" />
