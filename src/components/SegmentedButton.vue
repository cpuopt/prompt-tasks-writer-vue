<template>
  <div
    @contextmenu.prevent="contextmenu_status = true"
    @blur="delayLoseFocus"
    tabindex="0"
  >
    ...
    <el-input
      ...
      size="small"
      v-model="edit_input"
      :style="{ width: `${item.length + 2}rem` }"
      @keyup.enter.native="save_class"
    >
      <template #suffix>
        <el-icon
          class="el-input__icon"
          style="cursor: pointer; color: #606266"
          @click.stop="save_class"
          ><save
        /></el-icon>
      </template>
    </el-input>
...
const save_class = () => {
  ...
  if (props.deepth == 0) {
    ...
    props.tags.value.set(
      props.topChoice,
      renameMapKeyPreserveOrder(props.container, oldKey, newKey)
    );
  }
