...
<template>
    <el-tour v-model="TOUR">
        ...
        <el-tour-step target=".task-card:nth-child(1) .card-header .button-rt-container > button:nth-child(1) "
            title="折叠按钮" description="任务面板也可以折叠" />
        ...
        <el-tour-step target=".task-card:nth-child(1) .card-header .button-rt-container > div:nth-child(3) "
            title="删除该任务" description="删除这个任务，当任务列表仅有一个任务时，删除按钮将被禁用" />
    ...
    <div
        style="display: flex; position: fixed; top: 0px; right: 20vw; height: 30px; width: 60px; flex-direction: row; z-index: 2000">
        ...
        <el-tooltip v-if="IMPORTMASK_SHOW" class="box-item" style="" effect="dark" content="Import mask"
            placement="bottom">
        ...
        <el-container v-show="panelShow.show" :style="{}" class="pluginPanel" @click.stop @pointerdown.stop @wheel.stop>
            <el-header>
                <el-menu class="el-menu-demo" mode="horizontal" :ellipsis="false" style="align-items: center">
                    ...
                    <el-icon style="cursor: pointer; margin-inline: 0.5rem" color="#a6a9ad80" size="20"
                        @click="showTour()">
                        <QuestionFilled />
                    </el-icon>
                        ...
                        <el-card class="config-card" shadow="hover">
                            <template #header>
                                <div class="card-header">
                                    <div class="button-rt-container" style="flex-direction: row-reverse">
                                        <el-button size="small" class="button-rt" plain
                                            @click="CONFIG_SHOW = !CONFIG_SHOW"><el-icon :style="{
                            ...
                            <el-form label-width="9rem" style="max-width: 100%; margin-block: 20px" v-if="CONFIG_SHOW">
                                <el-form-item label="等待间隔：">
                                    <el-space :size="15">
                                        <el-input-number style="" v-model="tasklist.cooling" :min="2"
                                ...
                                <el-form-item label="实时保存插件数据：">
                                    <el-space :size="15">
                                        ...
                                        <el-button v-if="!tasklist.realTimeSave" type="primary" plain size="small"
                                            @click="manualSave()">{{
                                ...
                                <el-form-item label="清除插件数据：">
                                    <el-space :size="15">
                                        <ConfirmButton :text="'你确定要清除插件数据吗？'" :buttontext="'清除'" :confirm="'清除'"
                                            :cancel="'取消'" :panelShow="panelShow" :args="[removeTasklist]" :action="(...args) => {
                                                    args[0]();
                                                }
                                                "></ConfirmButton>
                                ...
                                <el-form-item label="禁用彩带动画：">
                                    <el-space :size="15">
                                        <el-switch v-model="tasklist.removeRunningAnmition" inline-prompt
                                ...
                                <el-form-item label="自动内存释放：">
                                    <el-space :size="15">
                                        <el-switch v-model="tasklist.blobMemoryRelease" inline-prompt
                                            :active-icon="Check" :inactive-icon="Close" />
                                        <ToolTip
                  ...
                  <el-space :size="15">
                    <el-button type="info" plain @click="createObjectURLInterceptor.revokeAll()" size="small">立即释放</el-button>
                                ...
                                <el-form-item label="启用颜色选择器：">
                                    <el-space :size="15">
                                        <el-switch v-model="colorPicker_status" inline-prompt :active-icon="Check"
                                            :inactive-icon="Close" />
                                        <ToolTip
                                ...
                                <el-form-item label="标签编辑器：">
                                    <el-space :size="15">
                                        <el-button type="info" plain @click="tag_selector_status.show = true"
                                ...
                                <el-form-item label="启用角色提示词：">
                                    <el-space :size="15">
                                        <el-switch v-model="tasklist.character" inline-prompt :active-icon="Check"
                                            :inactive-icon="Close" />
                  ...
                  <el-space :size="15">
                    ...
                    <div style="width: 99%">
                        <TransitionGroup name="list" tag="div">
                            <div v-for="(task, taskIndex) in tasklist.tasks" :key="task.uuid"
                                class="textarea2button_container" style="position: relative">
                                <el-card style="min-width: 600px" body-style="position: relative;" header-style=""
                                    ...
                                    <template #header>
                                        <div class="card-header" style="">
                                            ...
                                            <div class="button-rt-container" style="flex-direction: row-reverse">
                                                <el-button size="small" class="button-rt" plain
                                                    @click="task.fold = !task.fold"><el-icon :style="{
                                                        transition: 'transform 0.2s ease',
                                                ...
                                                <div>
                                                    <DeleteTask :task="task" :tasks="tasklist.tasks"
                                                        :panelShow="panelShow" :taskIndex="taskIndex"
                                                        :disable="tasklist.tasks.length < 2 || (activateTaskNum < 2 && task.activate == true)" />
                                                </div>

                                                <el-checkbox v-model="task.activate"
                    ...
                    <el-button class="long_button" plain size="small" :icon="ArrowDownBold" @click.stop="task.fold = false"></el-button>
                                    ...
                                    <el-form :model="task" label-width="8rem"
                                        style="max-width: 100%; margin-block: 20px" v-if="!task.fold">
                                        <el-form-item label="提示词组合方式：">
                                            <el-space :size="15">
                                                <el-switch v-model="task.random"
                                                    style="--el-switch-on-color: #13ce66; --el-switch-off-color: #409eff"
                                                    :active-value="true" :inactive-value="false" active-text="随机"
                                                    inactive-text="轮询" />
                                        ...
                                        <el-form-item label="尺寸设置：">
                                            <div style="display: flex; flex-direction: column; flex-wrap: nowrap">
                                                <el-space :size="15" v-for="(items, key) in task.size" :key="key"
                      ...
                      <el-space :size="15"> </el-space>
                                            ...
                                            <el-card shadow="hover">
                                                <el-form :model="task" label-width="7rem">
                                                    <el-form-item label="类型：">
                                                        <el-space :size="15">
                                                            <el-switch v-model="task.prompts.splice"
                                                                style="--el-switch-on-color: #13ce66; --el-switch-off-color: #409eff"
                                                                :active-value="true" :inactive-value="false"
                                                                active-text="拼接" inactive-text="完整"
                                                    ...
                                                    <el-form-item v-if="task.prompts.splice" label="抽取顺序：">
                                                        <el-tooltip :enterable="false"
                                                            :content="task.prompts.random ? '从每组中随机抽取提示词片段' : '从每组中按顺序选取提示词片段'"
                                                            placement="right">
                                                            <el-switch v-model="task.prompts.random"
                                                                style="--el-switch-on-color: #13ce66; --el-switch-off-color: #409eff"
                                                                :active-value="true" :inactive-value="false"
                                                                active-text="随机" inactive-text="轮询" />
                                                        </el-tooltip>
                                                    </el-form-item>
                                                    <el-form-item v-if="task.prompts.splice" label="提示词片段组：">
                                                        <VueDraggable v-model="task.prompts.data"
                                                            class="flex flex-col gap-2 p-4 w-300px bg-gray-500/5 rounded"
                                                            target=".prompts-groups" :scroll="true" :animation="200"
                                                            style="width: 100%" handle=".groups-handle">
                                                            <TransitionGroup name="list" tag="div" style="width: 100%"
                                                                class="prompts-groups">
                                                                <div v-for="(prompts_group, index) in task.prompts.data"
                                                                    :key="prompts_group.uuid" style="width: 100%">
                                                                    <AddArea :index="index" :task_prompts="task.prompts"
                                                                        :idKey="prompts_group.uuid" />
                                                                    <div class="textarea2button_container"
                                                                        ...
                                                                        <el-card :style="{
                                                                            ...
                                                                            <div
                                                                                style="display: flex; flex-direction: row; line-height: normal; justify-content: space-between">
                                                                                <el-button type="primary" size="small"
                                                                                    @click="addPromptSplice(prompts_group.data)"
                                                                                    style=""
                                                                                    :icon="ArrowDownBold"></el-button>
                                                                                <LoadPresets :PRESETS="PRESETS"
                                                                        ...
                                                                        <div class="button-rt-container">
                                                                            <DeleteButton :uprompt="prompts_group"
                                                                                :uprompts-date="task.prompts.data"
                                                                                :panel-show="panelShow" />
                                                                            <el-tooltip class="box-item" effect="dark"
                                                                                content="复制提示词组" placement="right">
                                                                                <el-button type="primary" size="small"
                                                                                    class="button-rt" plain @click="
                                                                                        () => {
                                                                            ...
                                                                            <IgnoreButton
                                                                                :prompt_group="prompts_group" />

                                                                            <el-color-picker v-if="colorPicker_status"
                                                                                size="small"
                                                                                v-model="prompts_group.color"
                                                                                :predefine="predefineColors" @click.stop
                                                                                show-alpha />
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                        ...
                                                        <el-button-group
                                                            ...
                                                            <el-button type="primary" size="small" plain
                                                                ...
                                                                :icon="ArrowDownBold"
                                                                :style="{ width: '90%' }"></el-button>
                                                            <el-tooltip class="box-item" effect="dark"
                                                                content="粘贴先前复制的提示词组到此处" placement="top">
                                                                <el-button type="success" size="small" plain
                                                                    @click="pastePromptGroup(task.prompts.data)"
                                                                    :icon="Edit" :style="{ width: '10%' }"
                                                    ...
                                                    <el-form-item v-if="!task.prompts.splice" label="完整提示词组：">
                                                        <el-card style="display: inline-block" shadow="hover">
                                                            <TransitionGroup name="list" tag="div">
                                                                <div v-for="(prompt, index) in task.prompts.data"
                                        ...
                                        <el-form-item label="反向提示词：">
                                            <el-card style="display: inline-block" shadow="hover">
                                                <TransitionGroup name="list" tag="div">
                                                    <div v-for="(uprompt, index) in task.uprompts.data"
                                                        class="textarea2button_container" :key="uprompt.uuid"
                                                        style="position: relative">
                                                        <el-input v-model="task.uprompts.data[index].data"
                                                            :autosize="{ minRows: 2, maxRows: 4 }" type="textarea"
                                                            placeholder="输入完整的反向提示词" />
                                                        <div class="button-rt-container">
                                                            <el-button type="danger" size="small" class="button-rt"
                                                                plain @click="removechild(uprompt, task.uprompts.data)"
                                                ...
                                                <el-button type="primary" size="small"
                                                    ...
                                                    :style="{ width: '100%', display: 'block' }" :icon="ArrowDownBold"
                                                    plain></el-button>
                                            </el-card>
                                        </el-form-item>
                                        <el-form-item v-if="tasklist.character" label="角色提示词：">
                                            <el-card style="display: inline-block" shadow="hover">
                                                <VueDraggable v-model="task.character.data"
                                                    class="flex flex-col gap-2 p-4 w-300px bg-gray-500/5 rounded"
                                                    target=".charactors" :scroll="true" :animation="200"
                                                    handle=".charactor-prompt-handle">
                                                    <TransitionGroup name="list" tag="div" class="charactors">
                                                        <div v-for="(character, index) in task.character.data"
                                                            class="textarea2button_container" :key="character.uuid"
                                                            style="position: relative">
                                                            <el-button :disabled="!(task.character.data.length > 1)"
                                                                :icon="DCaret"
                                                                class="charactor-prompt-handle cursor-move button-small-square"
                                                                size="small" />
                                                            <el-input v-model="character.prompts.data"
                                                                :autosize="{ minRows: 2, maxRows: 4 }" type="textarea"
                                                                placeholder="输入角色提示词" />
                                                            <el-input v-model="character.uprompts.data"
                                                                :autosize="{ minRows: 2, maxRows: 4 }" type="textarea"
                                                                placeholder="输入角色反向提示词" />
                                                            <div class="button-rt-container">
                                                                <el-button type="danger" size="small" class="button-rt"
                                                                    plain
                                                                    @click="task.character.data = task.character.data.filter((_) => _.uuid !== character.uuid)"
                                                                    :icon="Delete"></el-button>
                                                            </div>
                                                        </div>
                                                    </TransitionGroup>
            ...
            <el-footer style="height: 80px; display: flex; flex-direction: column; padding-bottom: 1rem">
                ...
                <el-progress :style="{ display: 'flex', marginTop: 'auto', width: '100%', height: '50px' }"
                    :text-inside="true" :stroke-width="22" :percentage="progress.percentage" v-show="progress.start"
                    striped striped-flow :duration="progress_duration">
                ...
                <el-button-group :style="{
                    ...
                    <el-button type="warning" :style="{ width: '50%', height: '100%', color: 'white' }"
                        @click="runTaskList()" color="#4A6BFF" v-show="!progress.start && progress.now == 0">
                        启动任务列表
                    </el-button>
                    ...
                    <el-button type="primary" :style="{ width: '25%', height: '100%', color: 'white' }"
                        @click="inportFromFile()" color="#4CC2C2"> 从文件导入任务队列 </el-button>

                    <el-button type="primary" :style="{ width: '25%', height: '100%', color: 'white' }"
                        @click="download2file()" color="#4CC2C2"> 导出到文件 </el-button>
                </el-button-group>
                <form ref="json_form"
                    style="display: none; filter: alpha(opacity=0); opacity: 0; width: 0; height: 0; padding: 0">
...
import { Debug, insert, generate_promptList, timeFormat, PromptsBuilder, count_task_prompts_num, removechild, click_generate } from '@/pojo/NAIutils.js';
...
const tasklist = reactive(localStorage.getItem('tasklist') == null ? PromptsBuilder.newTasklist() : loadTaskListConfigFromStorage());
watch(
    () => tasklist,
    (newValue, oldValue) => {
...
const runTaskList = async () => {
    ...
    progress.promptList = generate_promptList(toRaw(tasklist.tasks), tasklist.character);
