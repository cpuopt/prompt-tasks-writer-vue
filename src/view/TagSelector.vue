    ...
    <el-container class="el-container tag-selector" @click.stop>
        <el-header>
            <el-menu class="el-menu-demo" mode="horizontal" :ellipsis="false" style="align-items: center">
                <el-text class="mx-1 disabled-select" type="primary" size="large" :line-clamp="1">{{ pluginConfig.name }} 内置Tag编辑器 v{{ pluginConfig.userscript.version }} </el-text>
                ...
                <el-button size="small" type="primary" plain @click="saveToJson()">保存为Json</el-button>

                <el-button size="small" type="primary" plain @click="inportFromFile()">从Json导入</el-button>
                <el-button type="danger" plain :icon="CloseBold" @click="tag_selector_status.show = false" :style="{ height: '30px', width: '30px' }" />
            </el-menu>
        </el-header>
            ...
            <div class="tag-inputer">
                <div class="input-container">
                    <el-scrollbar style="width: 100%" always>
                        <VueDraggable v-model="selectedTags" class="flex flex-col gap-2 p-4 w-300px bg-gray-500/5 rounded" target=".tags-groups" :scroll="true" :animation="200" handle=".tags-handle">
                            <TransitionGroup name="list" tag="div" class="tags-groups"
                                ><SelectedTag v-for="tag in selectedTags" :container="selectedTags" :tag="tag" :key="tag.raw" />
                                <div id="tags_end" style="height: 0px; overflow: hidden" key="__tags_end"></div>
                            </TransitionGroup>
                        </VueDraggable>
                    </el-scrollbar>
                </div>
                <div class="button-container">
                    <el-button size="small" type="primary" plain @click.stop="copy_tags_selecter()">复制到剪切板</el-button>

                    <el-button size="small" type="danger" plain @click.stop="clear_tags_selecter()">清空</el-button>
                    <el-button size="small" type="success" plain @click.stop="read_from_clipboard()">从剪切板导入</el-button>
                    <div style="margin-left: auto"></div>
                    <div class="button-group" style="padding-inline: 0.2rem; border-left: 1px dashed #dadada; margin-top: -1px">
                        <el-button v-if="preset_load_status == false" size="small" @click.stop="preset_load_status = true">加载预设</el-button>
                        <div v-else>
                            <el-select size="small" v-model="Tag_select" style="width: 160px" @change="PRESETS_CHANGE" placeholder="选择要加载的预设">
                                <el-option size="small" v-for="item in PRESETS.prompts" :key="item.name" :value="item.name">
                                    <span style="float: left">{{ item.name }}</span>
                                    <span style="float: right; color: var(--el-text-color-secondary); font-size: 13px" @click.stop="PRESETS_delete(item.name)"> 删除 </span>
                                </el-option>
                            </el-select>
                            <el-button size="small" type="danger" plain @click.stop="preset_load_status = false">取消</el-button>
                        </div>
                    </div>
                    <div class="button-group" style="border-left: 1px dashed #dadada; margin-top: -1px; padding-left: 0.2rem">
                        <el-button size="small" v-if="preset_input_status == false" @click.stop="preset_input_status = true">保存到预设</el-button>
                        <div v-else>
                            <el-input size="small" v-model="preset_input" style="width: 160px" placeholder="输入预设名称" @keyup.enter.native="save_preset_click()" @click.stop>
                                <template #suffix>
                                    <el-icon class="el-input__icon" style="cursor: pointer; color: #606266" @click.stop="save_preset_click()"><save /></el-icon>
                                </template>
                            </el-input>
                            <el-button
                                size="small"
            ...
            <div class="segmented-container" style="height: 75%">
                <div class="segmented-header">
                    <el-scrollbar @wheel.passive="handleTopWheel" ref="topscrollContainer">
                        <div style="display: flex">
                            <el-segmented @contextmenu.prevent v-model="topChoice.value" :options="topOptions" size="default" @change="topOptionsChange">
                                <template #default="{ item }">
                                    <SegmentedButton :item="item" :container="tags.value" :deepth="0" :tags="tags" />
                                </template>
                            </el-segmented>
                ...
                <div class="segmented-body" style="flex: 1">
                    <div class="segmented-container" style="height: 100%">
                        <div class="segmented-header">
                            <el-scrollbar @wheel.passive="handleSecondWheel" ref="secondscrollContainer">
                                <div ref="innerRef" style="display: flex">
                                    <el-segmented @contextmenu.prevent v-model="secondChoice.value" :options="secondOptions" size="default" @change="secondOptionsChange">
                                        <template #default="{ item }">
                                            <SegmentedButton :item="item" :container="tags.value.get(topChoice.value)" :deepth="1" :tags="tags" :topChoice="topChoice.value" />
                                        </template>
                                    </el-segmented>

                                    <OperationInput v-if="topChoice.value != ''" :choice="secondChoice" :container="tags.value.get(topChoice.value)" type="second" />
                                </div>
                            </el-scrollbar>
                        </div>
                        <div class="segmented-body" style="flex: 1">
                            <el-scrollbar>
                                <div class="tags-container" style="width: 99%">
                                    <TagItem
                                        v-for="tag in tags.value.get(topChoice.value) ? tags.value.get(topChoice.value).get(secondChoice.value) : []"
                                        :tag="tag"
                                        :tags="tags.value.get(topChoice.value).get(secondChoice.value)"
                                        :bgColor="selectedTags.includes(tag) ? '#909399' : '#40a0ffe3'"
                                        @click.stop="selectTag(tag)"
                                    />
                                    <div class="add-tag" v-if="topChoice.value && secondChoice.value && !add_tag_status" @click.stop="add_tag">
                                        <el-icon color="#dadada"><Plus /></el-icon>
                                    </div>
                                    <TagItem v-if="add_tag_status" :tag="{ local: '', raw: '' }" :tags="tags.value.get(topChoice.value).get(secondChoice.value)" :add="true" />
                                </div>
                            </el-scrollbar>
                        </div>
                    </div>
